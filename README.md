...
|---|---|---|
| [`apps/web`](apps/web) | **Yayında** | Statik site: ana sayfadaki dünya küresi, OSINT paneli ve yöntem sayfası. Derleme adımı yok; D3 ve topojson-client depoda barındırılır. Panelde MapLibre GL ile bir **vektör altlık prototipi** vardır; varsayılan kapalıdır, yalnızca `panel.html?basemap=…` ile açılır. Katman listesi: [apps/web/README.md](apps/web/README.md) |
...
|---|---|---|
| [`apps/web`](apps/web) | **Live** | The static site: the globe on the home page, the OSINT dashboard and the methodology page. No build step; D3 and topojson-client are vendored. The dashboard carries a **vector basemap prototype** on MapLibre GL: off by default, on only with `panel.html?basemap=…`. Layer list: [apps/web/README.md](apps/web/README.md) |
