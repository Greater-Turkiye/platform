...
- **Doğrulanmış hiçbir şey insan onayı olmadan yayımlanmaz** ([ADR 0007](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0007-human-in-the-loop-publishing.md)). Toplayıcının ilgi süzgecinden geçen adaylar ise `datasets`'in `auto-data` dalına **otomatik, doğrulanmamış** kayıt olarak yazılır ve sitede öyle görünür; başlık ve özetleri makineyle yazılır, Türk kuvvetleriyle ilgili hiçbir aday bu yoldan geçmez ([ADR 0023](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0023-automatic-unverified-records.md)).
...
[`.github/workflows/collect.yml`](.github/workflows/collect.yml) altı saatte bir, 05:23, 11:23, 17:23 ve 23:23 UTC'de (ve elle tetiklenerek) çalışır ([ADR 0025](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0025-collector-every-six-hours.md)): koşulları kayda geçmiş akışları toplar, güvenlik süzgecinden geçirir, tekrarları eler, ilgi süzgecini uygular ve kalan adayları tarihli tek bir konuya yazar (aynı günün sonraki çalışmaları ona yorum ekler). Konudaki hiçbir öğe doğrulanmış değildir; kararı insan verir. Tekilleştirme defteri `collector-state` dalında tutulur ([collectors/state](collectors/state)).
...
- **Nothing verified is published without human approval** ([ADR 0007](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0007-human-in-the-loop-publishing.md)). Candidates that pass the collector's relevance filter are written as **automatic, unverified** records to the datasets `auto-data` branch and shown on the site as such; their titles and summaries are machine-written, and no candidate concerning Turkish forces takes that path ([ADR 0023](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0023-automatic-unverified-records.md)).
...
[`.github/workflows/collect.yml`](.github/workflows/collect.yml) runs every six hours, at 05:23, 11:23, 17:23 and 23:23 UTC, and on demand ([ADR 0025](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0025-collector-every-six-hours.md)): it collects the feeds whose terms are on record, applies the safety filter, drops repeats, applies the relevance filter and writes what is left into a single dated issue (later runs on the same day comment on it). Nothing in that issue is verified; a human decides. The deduplication ledger lives on the `collector-state` branch ([collectors/state](collectors/state)).
