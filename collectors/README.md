...
> Durum: RSS/Atom toplayıcı, güvenlik filtresi, ilgi süzgeci ve **günlük zamanlanmış çalışma** hazır ve işliyor. 2026-09-20'de kaynak tabanı genişletildi: **yedi akış** (BM'nin beşi, Kremlin'in ikisi) insan inceleme kuyruğuna alınıyor; on altı akışın tamamı artık `datasets` kaynak siciline bağlı (`source_id`). İlgi tabloları Türkçe ve İngilizce'nin yanında **Rusça ve Arapça** terimler de taşıyor — o diller eklenmeden önce Rusça ve Arapça akışların **tamamı** konu dışı sayılıyordu. Çalışmanın tamamı `collector-state` dalına bir **parti dosyası** olarak yayımlanır; onu `gt-signals` ve `gt-ops.reviews` içine [`apps/ingest`](../apps/ingest) Worker'ı çeker. Hiçbir akış doğrudan ingest'e **gönderilmez** (`enabled: false`): her aday önce bir insandan geçer. / Status: the collector, the safety filter, the relevance filter and the **daily run** work. On 2026-09-20 the source base was widened: **seven feeds** now reach the human review queue, all sixteen carry a `source_id` from the datasets registry, and the relevance tables gained **Russian and Arabic** terms — without them every item from those feeds was scored off-topic. Nothing is sent straight to ingest: every candidate passes a human first.
>
