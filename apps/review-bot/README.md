...
5. `gt-ops.reviews` içinde satır olması: bugün kuyruğu dolduran yazıcı henüz yok (toplayıcılar `gt-signals`'a yazacak, ardından triyaj `reviews` satırlarını üretecek). O gelene kadar bot boş bir kuyruk gösterir. / Rows in `gt-ops.reviews`: the writer that fills the queue does not exist yet, so until it does the bot correctly shows an empty queue.
