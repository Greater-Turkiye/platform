...
Erişim tarihi / Accessed: 2026-09-19 · Durum tarihi / Status as of: **2026-09-13**
...
> **Durum / Status.** Her özellik `status` (`active` | `ended` | `unclear`) ve `status_as_of` taşır. 2026-09-13 itibarıyla dört bölgenin dördü de `ended`: bölge artık **ayrı bir Türk kontrol alanı olarak** mevcut değildir. Harita durumu etiketlemeli ve `ended` bölgeleri güncel kontrol gibi göstermemelidir (ör. soluk dolgu ve "sona erdi / ended" etiketi).
> Every feature has `status` (`active` | `ended` | `unclear`) and `status_as_of`. As of 2026-09-13 all four zones are `ended`: the zone no longer exists **as a distinct Turkish-controlled area**. The map should label the status and must not style `ended` zones as current control (e.g. a faded fill and an "ended / sona erdi" label).

> **Şematik / Schematic.** `schematic: true` olan bölgelerin (`tur-op-firat-kalkani`, `tur-op-bahar-kalkani`, `tur-op-baris-pinari`) sınırı yaklaşıktır: Türkiye bir koordinat listesi yayımlamamıştır; poligon, ilan edilen kapsama karşılık gelen idari birimlerden bu projenin kurduğu bir yapıdır. `tur-op-zeytin-dali` Afrin ilçesinin idari sınırıdır. `tur-op-bahar-kalkani`, ilan "İdlib" olarak yapıldığı ve koordinat verilmediği için **İdlib ilinin tamamıdır** (26 nahiye); bölge içindeki kontrol hattı çizilmez.
> Zones with `schematic: true` (`tur-op-firat-kalkani`, `tur-op-bahar-kalkani`, `tur-op-baris-pinari`) have an approximate extent: Türkiye has published no coordinate list, and the polygon is this project's construction from the administrative units matching the announced scope. `tur-op-zeytin-dali` is the administrative boundary of the Afrin district. `tur-op-bahar-kalkani` is the **whole of Idleb governorate** (26 sub-districts), because the announcement named "Idlib" and gave no coordinates; no line of control inside it is drawn.
...
| `tur-op-bahar-kalkani` | Bahar Kalkanı | 01.03.2020 | `ended` (2026-09-13) | 5.392 km² | **Şematik** — İdlib ili (26 nahiye) | MSB açıklaması 01.03.2020 (BM Şartı md. 51; Adana, Astana, Soçi çerçevesi); 06.03.2020 Türkiye–Rusya ateşkesi |
...
- **Bahar Kalkanı — `ended`.** Harekât 27 Şubat 2020'de başladı, 1 Mart 2020'de Millî Savunma Bakanı tarafından adlandırıldı ve 5 Mart 2020 Moskova görüşmesinin ardından 6 Mart 2020'de yürürlüğe giren Türkiye–Rusya ateşkesiyle sona erdi (Anadolu Ajansı). İlanda koordinat verilmediği için alan İdlib ilinin tamamıdır; bölge içindeki kontrol hattı, gözlem noktaları ve birlikler çizilmez.
...
2. **Bölge başına nahiyeler** (`admin_areas`):
   ...
   - Bahar Kalkanı: İdlib ilinin 26 nahiyesinin tamamı (İdlib, Ebu Duhur, Binniş, Sarakib, Teftenaz, Maarettemsrin, Sermin, Maarretünnuman, Han Şeyhun, Sincar, Kefernebil, Tamania, Heyş, Harim, Dana, Salkin, Kefertaharim, Kurkina, Armanaz, Cisrüşşuğur, Bedama, Darkuş, Cenudiye, Erîha, Ehsim, Muhambil); derinlik sınırı uygulanmadı.
...
- **Bahar Kalkanı:** ilan "İdlib" olarak yapılmıştır ve bir koordinat listesi yoktur; il sınırı, ilanın çerçevesinin idari karşılığı olarak kullanıldı (şematik). Harekâtın fiilî etkisi ilin tamamını kapsamaz ve il sınırı bir kontrol hattı değildir.
...
| `tur-op-bahar-kalkani` | Spring Shield | 2020-03-01 | `ended` (2026-09-13) | 5,392 km² | **Schematic** — Idleb governorate (26 sub-districts) | MoND statement 1 Mar 2020 (UN Charter Art. 51; Adana, Astana, Sochi framework); Türkiye–Russia ceasefire 6 Mar 2020 |
...
- **Spring Shield — `ended`.** The operation began on 27 February 2020, was named by the Minister of National Defence on 1 March 2020 and ended with the Türkiye–Russia ceasefire that took effect on 6 March 2020 after the Moscow talks of 5 March (Anadolu Agency). The announcement carried no coordinates, so the area is the whole of Idleb governorate; no line of control, observation post or unit inside it is drawn.
...
2. **Sub-districts per zone** (`admin_areas`):
   ...
   - Spring Shield: all 26 sub-districts of Idleb governorate (Idleb, Abul Thohur, Bennsh, Saraqab, Teftnaz, Maaret Tamsrin, Sarmin, Ma'arrat An Nu'man, Khan Shaykun, Sanjar, Kafr Nobol, Tamanaah, Heish, Harim, Dana, Salqin, Kafr Takharim, Qourqeena, Armanaz, Jisr-Ash-Shugur, Badama, Darkosh, Janudiyeh, Ariha, Ehsem, Mhambal); no depth limit applied.
...
- **Spring Shield:** the announcement named "Idlib" and there is no coordinate list; the governorate boundary is used as the administrative equivalent of the announced frame (schematic). The operation's actual reach was not the whole governorate, and the governorate boundary is not a line of control.
...
- `tur-operation-areas.geojson`: 4 features, 641 vertices (Fırat Kalkanı 160, Zeytin Dalı 82, Bahar Kalkanı 270, Barış Pınarı 129). Each feature is a single `Polygon` without holes; every geometry `is_valid`; 3 decimals. Exterior rings counter-clockwise (RFC 7946), checked independently with the shoelace formula. A second run reproduces the file byte for byte.
- Inside Syria as drawn by the site (`countries-50m.json`, id 760): 100.0000 % for all four (area outside 0.000 km²); ∩ Iraq (id 368) 0 km²; ∩ Türkiye (id 792) 0 km².
- Overlap between zones: 0.000 km²; the Olive Branch – Euphrates Shield edge is shared exactly.
- Areas (geodesic, WGS84): Euphrates Shield 2,291 km² (8 sub-districts 2,330 km²; Türkiye's figure 2,015 km²); Olive Branch 1,736 km² (Afrin district 1,841 km²); Spring Shield 5,392 km² (26 Idleb sub-districts 5,416 km², clipped to the site's Syria 5,394 km²); Peace Spring 4,985 km² (4 sub-districts 7,666 km²; within 32 km of Türkiye 5,049 km²).
- Spring Shield (added 2026-09-19): Idleb governorate, every one of the 26 sub-district p-codes present in the pinned HDX build and its name checked; a single part, no holes; overlap with the other three zones 0.000 km² (it borders Olive Branch along the Afrin district edge).
