...
> Polygons carrying `coast_bridge_km` (the Black Sea, the Sea of Marmara and the Straits, the two schematic areas and the merged area) are extended by **up to 6 km** to the 1:50m coastline the site draws. Their own source coastlines are finer than the site's, and the schematic areas are additionally held back 600 m from land so they can be simplified; the difference showed as an unpainted strip along the coast once the map could be zoomed in (measured: only 48% of the Turkish sea coast was covered, median gap 0.30 km, p90 2.03 km). The extension runs along the **Turkish coast** only (plus the TRNC coast for the merged area), never onto land, and never past the median in a channel narrower than 12 km — Bodrum–Kos, Kaş–Kastellorizo get nothing. It is a **rendering correction, not a new claim**: the strip added is water the area already covered.
