...
> **Kıyı birleştirme / Coast bridge.** `coast_bridge_km` taşıyan poligonlar (Karadeniz, Marmara ve Boğazlar, iki şematik alan, birleşik alan), sitenin çizdiği 1:50m kıyı çizgisine kadar **en çok 6 km** genişletilmiştir. Gerekçe: alanların kaynak kıyı çizgileri (Marine Regions, Natural Earth 10m) siteninkinden daha incedir ve şematik alanlar sadeleştirilebilmek için karadan 600 m geri çekilmiştir; ikisi arasındaki fark, yakınlaştırıldığında kıyı boyunca boyanmamış bir şerit olarak görünüyordu (ölçüm: Türkiye deniz kıyısının yalnızca %48'i kapsanıyordu, medyan boşluk 0,30 km, p90 2,03 km). Genişletme yalnızca **Türkiye kıyısı** boyunca yapılır (birleşik alanda ayrıca KKTC kıyısı), kara üzerine taşmaz ve iki devlet arasındaki dar boğazlarda orta hattı geçmez — Bodrum–İstanköy, Kaş–Meis gibi 12 km'den dar geçitlerde hiç ekleme yapılmaz. Bu bir **çizim düzeltmesidir**, yeni bir yetki alanı iddiası değildir: eklenen şerit, alanın zaten kapsadığı sudur.
> Polygons carrying `coast_bridge_km` (the Black Sea, the two schematic areas and the merged area) are extended by **up to 6 km** to the 1:50m coastline the site draws. Their own source coastlines are finer than the site's, and the schematic areas are additionally held back 600 m from land so they can be simplified; the difference showed as an unpainted strip along the coast once the map could be zoomed in (measured: only 48% of the Turkish sea coast was covered, median gap 0.30 km, p90 2.03 km). The extension runs along the **Turkish coast** only (plus the TRNC coast for the merged area), never onto land, and never past the median in a channel narrower than 12 km — Bodrum–Kos, Kaş–Kastellorizo get nothing. It is a **rendering correction, not a new claim**: the strip added is water the area already covered.
...
| `tur-kktc-med-merged` | Çoklu poligon | `schematic` | Türkiye + KKTC birleşik tutumu | **Bu projenin yapısı**: `tur-med-schematic` ∪ ruhsat sahaları A–G ∪ KKTC 12 dm karasuları (şematik) — bkz. [KKTC ruhsat sahaları](#kktc-ruhsat-sahaları--trnc-licence-areas) |
...
| `tur-kktc-med-merged` | MultiPolygon | `schematic` | Combined Türkiye + TRNC position | **This project's construction**: `tur-med-schematic` ∪ licence areas A–G ∪ TRNC 12-nm territorial sea (schematic), see [TRNC licence areas](#kktc-ruhsat-sahaları--trnc-licence-areas) |
...
| `tur-med-schematic` | MultiPolygon | ≈ 162 200 km² | 497 | 1, 4 |
| `tur-kktc-med-merged` (bkz. / see [KKTC](#kktc-ruhsat-sahaları--trnc-licence-areas)) | MultiPolygon | ≈ 194 000 km² | 759 | 2, 4 |
...
4. **Genelleştirme.** Çıkarılacak küme (kara + yabancı karasuları) 600 m büyütüldü ve 450 m toleransla sadeleştirildi (Douglas–Peucker, LAEA metrik izdüşümde), ardından alandan çıkarıldı. Böylece alan karaya/yabancı karasularına hiçbir yerde değmez (en az ~145 m boşluk), resmî çizgiler (A/74/550, Mutabakat) ise olduğu gibi kalır. 5 km²'den küçük kopuk cepler atıldı (Ege: 4 cep, 0,8 km²; Akdeniz: 3 cep, 4,5 km²). IHO Ege Denizi veya Doğu Akdeniz havzasına değmeyen parçalar da atılır (bkz. Uyarılar, "Düzeltme"). 1e-4° ızgaraya oturtuldu; `is_valid` doğrulandı.
...
- **Düzeltme (Karadeniz parçası).** Önceki sürümde `tur-med-schematic` (ve dolayısıyla `tur-kktc-med-merged`), Sakarya açığında (41,09–41,12°K, 30,71–31,28°D) 140,5 km²'lik kopuk bir Karadeniz şeridi içeriyordu. Nedeni: zarfın kara üzerinden geçen 41°K kapanış kenarı (37°D → 22°D), metrik (LAEA) adımlarda düz bir kiriş olarak işlendiği için 31°D'de 41,23°K'ye kadar kavis yapıyor ve Karadeniz kıyısını kesiyordu. Artık yalnızca IHO Ege Denizi veya Doğu Akdeniz havzasına değen parçalar tutulur. Derleme ayrıca iki şematik alanın da 41°K'nin güneyinde kaldığını ve Ege alanının Marmara'ya en fazla Çanakkale kapanış hattı boyunca değdiğini denetler. Ege alanı değişmedi: Karadeniz'de veya Marmara'da kopuk parçası yoktur ve en kuzey noktası 40,75°K'dir (Meriç ağzı).
...
| `tur-kktc-med-merged` | — | — | — | ≈ 194,040 km² | 759 (2 parça / parts, 4 delik / holes) |
| ↳ KKTC 12 dm karasuları / TRNC 12-nm territorial sea (şematik / schematic, birleşik alanın içinde / inside the merged area) | — | — | — | ≈ 7,082 km² (+1,888 km² yeni / new) | — |
...
**Birleşik alan (`tur-kktc-med-merged`, şematik).** `tur-med-schematic` ile kırpılmış yedi sahanın birleşimi (dissolve). Bileşenler arasında 2 × 300 m'den dar şeritler, yalnızca sahaların 3 km yakınında morfolojik kapamayla dolduruldu (33,4 km²): (1) sahaların kuzey kenarları (WGS84'e çevrilmiş) ile sitedeki A/74/550 bölüm A çizgisi arasındaki 79–121 m'lik fark (aşağıdaki datum uyarısı), (2) B sahasının kuzey kenarı B4→B1, A/74/550 bölüm A'nın iki parçası üzerindeki iki noktayı birleştiren bir kiriştir ve 5. noktanın 360 m güneyinden geçer. Birleşimle kapanan ve `tur-med-schematic`'te olmayan delikler dolduruldu: Karpaz açığında, 2011 hattının kuzeyinde kalan Kıbrıs 12 dm karasuları kesiti (23,2 km² ve 9,1 km²; ayrıca 7 adet < 0,1 km² kenar artığı). Gerekçe: `tur-med-schematic` bu suları A/74/550'deki "yabancı karasuları hariç" ifadesi gereği çıkarmıştı; Türkiye + KKTC birleşik tutumunda KKTC kıyısı açığındaki sular "yabancı" değildir. Yunan adalarının karasuları delikleri (Meis vb.) korundu. Ardından Natural Earth 10m kara çıkarıldı. Özgün `tur-med-schematic` ve yedi saha ayrıca dosyada kalır; ruhsat sahalarının geometrisi değişmez.

**KKTC karasuları (ŞEMATİK — bu projenin yapısı, resmî koordinat değildir).** Birleşik alanın üçüncü bileşeni, KKTC kıyısından ölçülen 12 deniz millik karasularıdır. Önceki sürümde birleşik alan, Kıbrıs 12 dm karasularını (Marine Regions) KKTC kıyısı boyunca da dışarıda bırakıyordu; bu yüzden kıyı ile ruhsat sahaları arasında (kuzeyde B ve C sahalarının güney kenarı ile kıyı arası, Karpaz ucu, doğuda Mağusa'ya kadar) boş bir kuşak kalıyordu. Türkiye + KKTC birleşik tutumunda KKTC'nin kendi karasuları alana dahildir.
1. **Kıyı.** Natural Earth 10m admin-0 "N. Cyprus" (`CYN`) poligonunun kıyısı: ana kara ve Erenköy/Kokkina dış parçası. Adanın geri kalanı: "Cyprus" (`CYP`), BM ara bölgesi (`CNM`), Dhekelia (`ESB`), Akrotiri (`WSB`). Kara sınırı (ateşkes hattı/ara bölge kenarı) kıyı sayılmadı. Sitenin çizdiği Natural Earth 50m "N. Cyprus" (`countries-50m.json`) yerine **10m** seçildi, çünkü: derlemedeki tüm kara işlemleri (kara çıkarma, şematik alanlar, ruhsat kırpma) 10m ile yapılır; 10m, Erenköy dış parçasını ve BM ara bölgesini ayrı birimler olarak içerir (50m'de yoktur); ve yaklaşık 5 kat daha ayrıntılıdır. 50m ve 10m kıyıları arasında yer yer birkaç yüz metrelik fark vardır.
2. **Genişlik.** 12 dm (22 224 m), normal esas hattan (düz esas hat yok), Kıbrıs merkezli azimut eşit uzaklık izdüşümünde (ölçek hatası < 1e-4).
3. **Eşit uzaklık ayrımı.** KKTC ve GKRY kıyılarının 12 dm kuşaklarının örtüştüğü yerde, iki kıyı arasındaki eşit uzaklık hattı kullanıldı. Kıyılar 200 m aralıkla örneklendi (1 630 KKTC + 1 580 diğer nokta) ve Voronoi hücreleri alındı. Hat, kara sınırının kıyıdaki uçlarından başlar: batıda Yeşilırmak/Lefke yakınında (35,1816K 32,7114D), Erenköy çevresinde (35,1786K 32,6019D) ve doğuda Mağusa/Derinya yakınında (35,0638K 34,0123D). Hattın toplam uzunluğu 93,3 km'dir; 1 km aralıklı örneklerde iki kıyıya uzaklık farkı < 1 m'dir.
4. **Kara.** 50 m geri çekilerek (40 m sadeleştirme) çıkarıldı.
5. **Sonuç.** 2 parça (ana kıyı ≈ 6 973 km², Erenköy çevresi ≈ 109 km²), toplam ≈ **7 082 km²**. Bunun ≈ **1 888 km²**'si birleşik alanda daha önce yoktu. Erenköy'ün karasuları, çevresi GKRY tarafı 12 dm ile ayrıldığından birleşik alanda ayrı bir parçadır.
6. **Değişmeyenler.** GKRY tarafının 12 dm'si ve Yunan adalarının karasuları delikleri değiştirilmedi. Şerit ve delik dolguları GKRY tarafı 12 dm'ye alan eklemez; derleme bunu denetler (eşik 0,5 km²). KKTC 12 dm kuşağında birleşik alanın dışında kalan su toplam 15,9 km²'dir ve tümü ≤ 120 m genişliğindeki şeritlerdir (50 m kara payı ve yuvarlama).

Sonuç: 2 parça (ana alan ve Erenköy çevresi), 4 delik, 759 köşe, ≈ **194 040 km²** (KKTC karasuları +1 888 km²; Sakarya açığındaki hatalı Karadeniz parçası −140 km², bkz. [Şematik alanlar](#şematik-alanlar--schematic-areas)). Önizleme: `tools/geo/kktc-preview.png`.
...
**Merged area (`tur-kktc-med-merged`, schematic).** Union (dissolve) of `tur-med-schematic` and the seven clipped areas. Slivers narrower than 2 × 300 m between the components are filled by morphological closing, only within 3 km of the licence areas (33.4 km²). They come from two sources. (1) The 79–121 m offset between the areas' northern edges (converted to WGS84) and the A/74/550 section A line as drawn on the site (see the datum caveat below). (2) Block B's northern edge B4→B1 is a chord between two points on two segments of section A and passes 360 m south of point 5. Holes that only close because of the merge, and that `tur-med-schematic` does not have, are filled. These are the part of the Cyprus 12-nm territorial-sea cut-out north of the 2011 line off Karpas (23.2 km² and 9.1 km², plus 7 edge remnants < 0.1 km²). Rationale: `tur-med-schematic` removed these waters because A/74/550 says "except foreign territorial waters"; in a combined Türkiye + TRNC position, waters off the TRNC coast are not foreign. The Greek islands' territorial-sea holes (Kastellorizo/Meis etc.) are kept. Natural Earth 10m land is then removed. The original `tur-med-schematic` and the seven areas remain in the file; the licence geometries are unchanged.

**TRNC territorial sea (SCHEMATIC — this project's construction, not official coordinates).** The third component of the merged area is a 12-nautical-mile territorial sea measured from the TRNC coast. In the previous version the merged area also cut out the Cyprus 12-nm territorial sea (Marine Regions) along the TRNC coast. That left an empty band between the coast and the licence areas: north between the southern edges of blocks B and C and the coast, at the Karpas tip, and east towards Famagusta. In a combined Türkiye + TRNC position the TRNC's own territorial waters belong to the area.
1. **Coast.** The coast of the Natural Earth 10m admin-0 "N. Cyprus" (`CYN`) polygon: the main area and the Erenköy/Kokkina exclave. The rest of the island is "Cyprus" (`CYP`), the UN buffer zone (`CNM`), Dhekelia (`ESB`) and Akrotiri (`WSB`). The land border (ceasefire line / buffer-zone edge) does not count as coast. **10m** was chosen over the Natural Earth 50m "N. Cyprus" the site renders (`countries-50m.json`) for three reasons: every land operation in the build (land removal, schematic areas, licence clipping) uses 10m; 10m has the Erenköy exclave and the UN buffer zone as separate units (50m has neither); and it is about 5× more detailed. The 50m and 10m coasts differ by up to a few hundred metres in places.
2. **Breadth.** 12 nm (22,224 m) from the normal baseline (no straight baselines), measured in an azimuthal equidistant projection centred on Cyprus (scale error < 1e-4).
3. **Equidistance split.** Where the 12-nm belts of the TRNC and Greek Cypriot coasts overlap, they are divided by the equidistance line between the two coasts. Both coasts were sampled every 200 m (1,630 TRNC + 1,580 other points) and Voronoi cells taken. The line starts at the coastal ends of the land border: west near Yeşilırmak/Lefke (35.1816°N 32.7114°E), around Erenköy (35.1786°N 32.6019°E), and east near Famagusta/Deryneia (35.0638°N 34.0123°E). It is 93.3 km long in total; at 1-km samples the distances to the two coasts differ by < 1 m.
4. **Land.** Removed with a 50 m set-back (40 m simplification).
5. **Result.** 2 parts (main coast ≈ 6,973 km², around Erenköy ≈ 109 km²), total ≈ **7,082 km²**. Of this, ≈ **1,888 km²** was not yet in the merged area. Erenköy's territorial sea is a separate part of the merged area because Greek Cypriot-side 12-nm waters surround it.
6. **Unchanged.** The Greek Cypriot side's 12 nm and the Greek islands' territorial-sea holes are unchanged. The sliver and hole fills add no area in the Greek Cypriot-side 12 nm; the build checks this (threshold 0.5 km²). Water in the TRNC 12-nm belt still outside the merged area totals 15.9 km², all in strips ≤ 120 m wide (the 50 m land set-back and rounding).

Result: 2 parts (the main area and the part around Erenköy), 4 holes, 759 vertices, ≈ **194,040 km²** (TRNC territorial sea +1,888 km²; the stray Black Sea piece off Sakarya −140 km², see [Schematic areas](#şematik-alanlar--schematic-areas)). Preview: `tools/geo/kktc-preview.png`.
...
| Natural Earth 10m admin-0 countries; 10m minor islands | Public domain | https://naciscdn.org/naturalearth/10m/cultural/ne_10m_admin_0_countries.zip · https://naciscdn.org/naturalearth/10m/physical/ne_10m_minor_islands.zip | Schematic areas: mainland/island classification, coasts for the distance transform, land-border termini; TRNC territorial sea: "N. Cyprus" (`CYN`) coast vs `CYP`, `CNM`, `ESB`, `WSB` |
...
- Coast bridge (`bridge_coast.py`, 2026-09-19): 4 features bridged — Black Sea +738 km², Aegean schematic +2,086 km², Mediterranean schematic +1,410 km², merged Türkiye + TRNC +1,543 km². **The Sea of Marmara and the Straits are not bridged** (`SKIP`): that polygon already covers both Straits on purpose, because they are narrower than the 1:50m coastline, and widening it further only paints more of İstanbul blue at close zoom. Land overlap per feature after the step: Black Sea 1,212 km², Marmara 463 km², Aegean 781 km², Mediterranean 424 km², merged 543 km² — all of it from the sources' own finer coastlines, which is why the panel fades the maritime fills once it is zoomed past what 1:50m can support. Coverage of the Turkish sea coast (800 samples along the 1:50m coastline, excluding land borders): 48% → 100% (798/800), median gap 0.30 km → 0.000 km, p90 2.03 km → 0.004 km, worst remaining 1.07 km at the Evros/Meriç mouth and 0.91 km in the Bodrum–Kos channel, both where the edge is a boundary with another state and not an artefact. Land cover added by the bridge: ≤ 1.4 km² per feature, all of it grid snapping at 1e-4° along coastlines over a thousand kilometres long (`SNAP_SLACK_KM2 = 5`). The step is idempotent: a second run reports no change (`--check` exits 0).
