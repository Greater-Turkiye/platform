...
| Stratejik program ortağı / Programme partner | `GT.PARTNERS` katmanı `strategic` | Rusya: Akkuyu (6007 s. Kanun) ve S-400; kehribar ve kesikli — ittifak değil / Russia: Akkuyu and the S-400, amber and dashed so it cannot read as an ally |
| Rozetler / Badges | `GT.partnerBadges` + `GT.mapDefs` glifleri | Ülke adının altında en çok üç glif: NATO, kademe, TSK varlığı, program. Aynı tablonun ikinci okunuşu; yeni bir iddia yok / at most three glyphs under a country's name, read from the same tables as the fills |
...
| Olaylar / Events | `../datasets/*.jsonl` | Koordinatsız olaylar bölge başına tek bir sayılı halkada toplanır, tıklanınca bölgenin listesi açılır / events without coordinates are counted in one numbered ring per region, which opens that region's list |
