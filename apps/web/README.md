...
- `panel.html` — OSINT paneli: yakınlaştırılabilir harita, **bölge/tür/durum/dönem** filtreleri, arama, katmanlar (Kayıtlar ve Harita olarak gruplu), listenin üstünde **dört satırlık durum özeti** (dönemdeki kayıt sayısı, doğrulama dağılımı, öne çıkan bölge, son kayıt), açılır **harita anahtarı** (kapalı katmanın satırı gösterilmez) ve **brifing çıktısı** (`Brifing` düğmesi: başlık + süzgeç + zaman damgası, harita, özet ve liste; ekranda olan neyse kâğıda o gider). Derin bağlantı: `panel.html?region=aegean`, `panel.html?period=30`, `panel.html?id=evt_…`. / The OSINT dashboard: zoomable map, region/type/status/**period** filters, search, grouped layer switches, a four-line **situation summary** above the list, a **collapsible map key** that hides rows whose layer is off, and a **briefing print** (header, filter, timestamp, map, summary, list).
...
- `ticaret.html` — **İsrail ticareti**: Türkiye'nin 2024'te ilan ettiği ticaret durdurma kararı ile iki devletin kendi aylık beyanlarının karşılaştırması (`assets/data/trade-il.json`). İki çizgi, aralarındaki fark, ve Türkiye'nin komşu satırlarının yasak öncesi/sonrası 12 aylık ortalamaları. Sayfa ayrıca, yaptırım listelerinden **gemi kimliği ve kayıtlı önceki bayrak** bölümü taşır. Hiçbir gemi izlenmez ([ADR 0022](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0022-trade-compliance-not-vessel-tracking.md)). / **Trade with Israel**: Türkiye's declared halt against both states' own monthly returns; two lines, the gap between them, and the neighbours' lines before and after. No vessel is tracked.
- `deniz.html` — **Deniz paneli**: sitenin tek açık temalı sayfası. Türkiye'nin deniz yetki alanları, Boğazlar, KKTC ruhsat sahaları ve Türk adaları; altlarında 2015–2021 ilan edilen askerî faaliyetin ızgarası (`msi-density.json`). Haritanın altında üç okuma: denize göre ilan edilen faaliyet, `vessels-sanctioned.json`'dan bayrak değişiklikleri ve `trade-il.json`'dan durdurulan ticaretin denizyolu payı. D3 ile çizilir, altlık karo kullanmaz. **Hiçbir geminin konumu yoktur** ([ADR 0022](https://github.com/Greater-Turkiye/handbook/blob/main/decisions/0022-trade-compliance-not-vessel-tracking.md)). / **The sea panel**: the one light page on the site — maritime jurisdiction, the Straits, the TRNC licence blocks and the Turkish islands over a wash of the activity announced at sea, with three readings under the map. Drawn with D3, no tiles, and no vessel positions.
